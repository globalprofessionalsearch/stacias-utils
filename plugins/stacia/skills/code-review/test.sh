#!/usr/bin/env bash
# Run the stacia-code-review plugin's test suites (vitest + the Python helper
# tests). Single entrypoint used by BOTH the git pre-commit hook and CI, so
# testing is gated the same way `summon lint` is. Installs node deps if
# missing; the Python tests use the stdlib runner (no pytest required).
set -euo pipefail
cd "$(dirname "$0")/coordinator"

if [ ! -d node_modules ]; then
  echo "› installing coordinator deps (npm ci)…"
